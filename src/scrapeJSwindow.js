...
const mdn = [ // [...document.querySelectorAll('dt > a > code')].map(e => e.textContent).map(s => { if (s.includes('.')) { return s.split('.')[1] } else return s }).map(s => s.replace('()', '')).join("', '")
  'DOMParser', 'Image', 'Option', 'StaticRange', 'Worker', 'XMLSerializer', 'clientInformation', 'closed', 'console', 'credentialless', 'customElements', 'crypto', 'devicePixelRatio', 'document', 'event', 'external', 'frameElement', 'frames', 'fullScreen', 'history', 'innerHeight', 'innerWidth', 'isSecureContext', 'length', 'location', 'locationbar', 'localStorage', 'menubar', 'messageManager', 'mozInnerScreenX', 'mozInnerScreenY', 'name', 'navigation', 'navigator', 'opener', 'outerHeight', 'outerWidth', 'pageXOffset', 'pageYOffset', 'parent', 'performance', 'personalbar', 'screen', 'screenX', 'screenLeft', 'screenY', 'screenTop', 'scrollbars', 'scrollMaxX', 'scrollMaxY', 'scrollX', 'scrollY', 'self', 'sessionStorage', 'sidebar', 'speechSynthesis', 'status', 'statusbar', 'toolbar', 'top', 'visualViewport', 'window', 'caches', 'indexedDB', 'origin', 'scheduler', 'content', 'defaultStatus', 'orientation', 'returnValue', 'alert', 'blur', 'cancelAnimationFrame', 'cancelIdleCallback', 'clearImmediate', 'close', 'confirm', 'dump', 'find', 'focus', 'getComputedStyle', 'getDefaultComputedStyle', 'getSelection', 'matchMedia', 'moveBy', 'moveTo', 'open', 'postMessage', 'print', 'prompt', 'queryLocalFonts', 'requestAnimationFrame', 'requestIdleCallback', 'resizeBy', 'resizeTo', 'scroll', 'scrollBy', 'scrollByLines', 'scrollByPages', 'scrollTo', 'setImmediate', 'setResizable', 'sizeToContent', 'showOpenFilePicker', 'showSaveFilePicker', 'showDirectoryPicker', 'stop', 'updateCommands', 'addEventListener', 'dispatchEvent', 'atob', 'btoa', 'clearInterval', 'clearTimeout', 'createImageBitmap', 'fetch', 'removeEventListener', 'setInterval', 'setTimeout', 'reportError', 'back', 'captureEvents', 'forward', 'home', 'releaseEvents', 'showModalDialog', 'error', 'languagechange', 'orientationchange', 'devicemotion', 'deviceorientation', 'resize', 'storage', 'vrdisplayconnect', 'vrdisplaydisconnect', 'vrdisplayactivate', 'vrdisplaydeactivate', 'vrdisplaypresentchange', 'animationcancel', 'animationend', 'animationiteration', 'animationstart', 'copy', 'cut', 'paste', 'offline', 'online', 'blur', 'focus', 'gamepadconnected', 'gamepaddisconnected', 'hashchange', 'pagehide', 'pageshow', 'popstate', 'beforeunload', 'DOMContentLoaded', 'load', 'unload', 'appinstalled', 'beforeinstallprompt', 'message', 'messageerror', 'afterprint', 'beforeprint', 'rejectionhandled', 'unhandledrejection', 'transitioncancel', 'transitionend', 'transitionrun', 'transitionstart', 'vrdisplayactivate', 'vrdisplayblur', 'vrdisplayconnect', 'vrdisplaydeactivate', 'vrdisplaydisconnect', 'vrdisplayfocus', 'vrdisplaypresentchange'
]

const filterOut = [
  'InstallTrigger', 'clientInformation', 'setResizable', 'u2f', 'ontoggle', 'ondragexit', 'onabsolutedeviceorientation', 'returnValue', 'messageManager', 'home', 'cut', 'paste', 'popstate', 'offline', 'beforeinstallprompt',  'gamepaddisconnected', 'transitionstart', 'vrdisplayfocus', 'animationcancel', 'unload', 'beforeprint', 'afterprint', 'transitioncancel', 'message', 'animationstart', 'gamepadconnected', 'animationiteration', 'animationend', 'copy', 'transitionrun', 'messageerror', 'pagehide', 'resize', 'vrdisplaydisconnect', 'appinstalled', 'vrdisplaypresentchange', 'transitionend', 'rejectionhandled', 'deviceorientation', 'unhandledrejection', 'languagechange', 'error', 'load', 'pageshow', 'vrdisplayblur', 'vrdisplayactivate', 'devicemotion', 'beforeunload', 'DOMContentLoaded', 'vrdisplaydeactivate', 'vrdisplayconnect', 'online', 'orientationchange', 'hashchange'
]
...
async function getNfo (p) {
  try {
    ...
    try {
      ...
      try {
        return await req(`https://developer.mozilla.org/en-US/docs/Web/API/EventTarget/${p}`)
      } catch (err) {
        return { error: 'axios bug' }
      }
    }
